...
public class EncodingDetect {
    ...
    public static String detect(String path) {
        return BytesEncodingDetect.javaname[encodingDetect.detectEncoding(new File(path))];
    }

    public static String detect(File file) {
        return BytesEncodingDetect.javaname[encodingDetect.detectEncoding(file)];
    }

    public static String detect(byte[] contents) {
        return BytesEncodingDetect.javaname[encodingDetect.detectEncoding(contents)];
    }

    public static String detect(URL url) {
        return BytesEncodingDetect.javaname[encodingDetect.detectEncoding(url)];
    }
    ...
    static class BytesEncodingDetect extends Encoding {
        ...
        public int detectEncoding(URL testurl) {
            byte[] rawtext = new byte[102400];
            int bytesread = 0, byteoffset = 0;
            int guess = OTHER;

            try (InputStream is = testurl.openStream()) {
        ...
        public int detectEncoding(File testfile) {
            ...
            try (InputStream fileis = new FileInputStream(testfile)) {
                fileis.read(rawtext);
            } catch (Exception ex) {
                throw new UnexpectedException(ex);
            }
